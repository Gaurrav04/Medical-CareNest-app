import { DataProps, getDoctorsBySpecialtySlug, getDoctorsBySymptomId} from '@/actions/doctors';
            ...
            <h2 className="capitalize font-semibold">Other Services</h2>
            {
              ...
              <div className="py-3 flex flex-col text-sm space-y-2">
               {services.map((service,i)=>{
                return (
                  <Link 
                  key={i} href={`/symptoms/${service.slug}?id=${service.id}`} className="hover:text-blue-600">
                  {service.title}
                </Link>
                )
               })}
