...
export default async function Page({ 
    ...
    <div className="container p-8">
        ...
        <div className="col-span-9">
          ...
          <h2 className="pb-3">Result for <span className="font-semibold">{query} </span>in services</h2>
          <ServiceList data={searchServices}/>
         </div>}
         {specialties && specialties.length > 0 && <div className="py-6 border-b">
          <h2 className="pb-3">Result for <span className="font-semibold">{query} </span> in Specialties</h2>
          <LinkCards className="bg-cyan-500" specialties={specialties}/>, 
         </div> }
         {symptoms && symptoms.length > 0 && <div className="py-6 border-b">
          <h2 className="pb-3">Result for <span className="font-semibold">{query} </span>in Symptoms</h2>
          <SymptomCards className="bg-cyan-500" symptoms={symptoms}/>, 
         </div>}
         {
            doctors && doctors.length > 0 && (
             <div className="py-6">
               <h2 className="pb-3">Result for <span className="font-semibold">{query} </span>in Doctors</h2>
